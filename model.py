...
def train_model(model, train_loader, val_loader, criterion, optimizer, device, num_epochs=10, model_name="model"):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0 or (epoch + 1) == num_epochs:  # Co 5 epoki lub po ostatniej
...
if __name__ == "__main__":
    ...
    for model_name, model in models_to_train.items():
        ...
        #model.load_state_dict(torch.load(f"Models\{model_name}_best_model.pt"))   # uncoment to use previous model to continue training
        model = model.to(device)
